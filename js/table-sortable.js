...
var defaultConfig = {
    ...
    sortingIcons: {
        asc: '<span><img src="../img/sortdown.gif"/></span>',
        dec: '<span><img src="../img/sortup.gif"/></span>',
        nosort:'<span><img src="../img/nosort.gif"/></span>',
    },
    nextText: '<span>上一頁</span>',
...
var TableSortable = function () {
    ...
    _createClass(TableSortable, [{
        ...
        value: function constructTableObject() {
            ...
            table.addClass('table-sm w-100 bg-white table-bordered table-click');
            return table;
        }
    }, {
        ...
        value: function addSorting(col, sorting, key, value, sortingIcons) {
            ...
            if (isArray(sorting)) {
                if (sorting.indexOf(key) !== -1) {
                    ...
                    col.append(sortingIcons.nosort);
                return col;
            }
        ...
        value: function sortData(sortKey) {
            if(this.config.serverSidePagination){
                if (this.config.events && isFunction(this.config.events.onQuery)) {
                    this.config.events.onQuery.call(this);
                }
                return;
            }
        ...
        value: function updateData() {
            if (this.config.events && isFunction(this.config.events.onEdit)) {
                this.config.events.onEdit.call(this);
            }
        }
    },{
        key: 'InsertData',
        value: function InsertData() {
            if (this.config.events && isFunction(this.config.events.onInsert)) {
                this.config.events.onInsert.call(this);
            }
        }
    },]);
